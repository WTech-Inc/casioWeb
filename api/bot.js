...
// ============ Doubao AI 核心邏輯 ============
let w1 = Math.random() * 0.2 - 0.1; // prompt p
let w2 = Math.random() * 0.2 - 0.1; // ans p
let b = 0.0;
...
function buildVocabulary() {
    ...
    for (const [prompt, [answer, _]] of Object.entries(datasets)) {
        // 處理提示詞
...
function textToVector(text) {
    if (vocabulary === null) buildVocabulary();
...
async function train(epochs = 100, learningRate = 0.01) {
    buildVocabulary();
    
    // 預計算所有觸發詞
    const allTriggers = [];
    for (const prompt of Object.keys(datasets)) {
        const triggers = prompt.replace(/\|/g, '｜').split('｜');
        triggers.forEach(trigger => {
    ...
    allTriggers.forEach(trigger => {
        triggerVectors[trigger] = textToVector(trigger);
    });
    ...
    for (let i = 0; i < allTriggers.length; i++) {
        for (let j = 0; j < allTriggers.length; j++) {
            if (i !== j) {
                ...
                const sim = combinedSimilarity(t1, t2);
    ...
    for (let epoch = 0; epoch < epochs; epoch++) {
        ...
        for (const { input, target, label } of trainingData) {
            ...
            const sim = cosineSimilarity(inputVec, targetVec);
            
            // 找到相關分數
            let relevance = 0.5;
            for (const [prompt, [_, rel]] of Object.entries(datasets)) {
                const triggers = prompt.replace(/\|/g, '｜').split('｜');
            ...
            // 前向傳播
            const z = sim * w1 + relevance * w2 + b;
            const prediction = relu(z);
            const error = prediction - label;
            
            // 反向傳播
            const gradient = error * reluDerivative(z);
            
            w1 -= learningRate * gradient * sim;
            w2 -= learningRate * gradient * relevance;
    ...
    console.log('✅ 訓練完成！');
    console.log(`最終權重: w1=${w1.toFixed(4)}, w2=${w2.toFixed(4)}, b=${b.toFixed(4)}`);
}

async function predict(userInput, threshold = 0.3) {
    // 特殊關鍵字處理
    ...
    if (hasRentKeyword) {
        ...
        const answers = datasets[prompt][0].replace(/\|/g, '｜').split('｜');
    ...
    for (const [prompt, [_, relevance]] of Object.entries(datasets)) {
        const triggers = prompt.replace(/\|/g, '｜').split('｜');
        
        for (const trigger of triggers) {
            const triggerVector = textToVector(trigger);
            const similarity = combinedSimilarity(userInput, trigger);
            const z = similarity * w1 + relevance * w2 + b;
            const score = relu(z);
    ...
    if (bestScore >= threshold && bestPrompt) {
        const answers = datasets[bestPrompt][0].replace(/\|/g, '｜').split('｜');
        const randomAnswer = answers[Math.floor(Math.random() * answers.length)];
        return {
            answer: randomAnswer,
            score: bestScore,
        ...
        // 調用搜索
        const searchResult = await baiduSearch(userInput);
        return {
            answer: searchResult,
            score: 0.0,
            source: "搜索結果"
...
client.once('ready', async () => {
    ...
    console.log(`🔗 邀請鏈接: https://discord.com/oauth2/authorize?client_id=${client.user.id}&scope=bot&permissions=8`);
    
    // 設置狀態
    client.user.setActivity('@doubao 需要幫助嗎？', { type: 'PLAYING' });
    
    // 開始訓練
    console.log('🧠 開始訓練 AI 模型...');
    await train(100, 0.01);
...
client.on('messageCreate', async (message) => {
    ...
    if (message.channel.type === 1 || isMentioned) {
        try {
            ...
            const { answer, score, source } = await predict(userMessage);
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (commandName === 'ping') {
        await interaction.reply(`🏓 Pong! 延遲: ${client.ws.ping}ms`);
    } else if (commandName === 'chat') {
        const message = interaction.options.getString('message');
        
        await interaction.deferReply();
        
        const { answer } = await predict(message);
